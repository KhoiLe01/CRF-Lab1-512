...
\paragraph{(4a):}
\noindent \textbf{Note:} Before figure~\ref{fig:final4a}, all the graphs contain accuracy instead of error. For analysis and reasoning on the hyperparameters, this is fine. In our final graph (figure~\ref{fig:final4a}), we will use error as the problem requires.\\
...
We can see the reduction in the noise and value of the objective by taking the log:
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
...
\noindent Note that the reason we divide the node count by $|S|$ is that each node is increases by some $p_c$ once, while for edge count it is double counted. In addition, we can draw sample at the end of each loop, but we can also do it separately in line 4 and 5 because the distribution are independent. We think that the count should be update right before the sampling because it will capture the relationship of the current node and the state it is in, while updating the count outside the loop is basically taking the average from the sample, which still allows room for high variance. Note that updating before or after the sampling make a slight difference where we sample from the first or last sampling, but it should not make a big difference.
\subsubsection*{(ii):}
