...
\stuinfo{\shortstack[l]{Angelo Zangari\\Khoi Le\\Javed Shah}}{\shortstack[l]{azang@uic.edu\\kle539@uic.edu\\jshah93@uic.edu}}
\graphicspath{{./}{./Figures/}}
...
\paragraph{(1a) Derivative with respect to $\wvec_c$.}
For a fixed training example $(X^t,\yvec^t)$, define
\begin{align}
S(X^t,\yvec)
=
\sum_{s=1}^m \inner{\wvec_{y_s}}{\xvec_s^t}
+
\sum_{s=1}^{m-1} T_{y_s,y_{s+1}}.
\end{align}
Then
\begin{align}
\log p(\yvec^t \mid X^t)
&=
\log \frac{1}{Z_{X^t}} \exp\!\bigl(S(X^t,\yvec^t)\bigr) \\
&=
S(X^t,\yvec^t) - \log Z_{X^t}.
\end{align}
Therefore,
\begin{align}
\grad_{\wvec_c}\log p(\yvec^t \mid X^t)
=
\grad_{\wvec_c} S(X^t,\yvec^t)
-
\grad_{\wvec_c}\log Z_{X^t}.
\end{align}

We now compute the two terms separately.
...
\grad_{\wvec_c}
\left(
\sum_{s=1}^m \inner{\wvec_{y_s^t}}{\xvec_s^t}
+
\sum_{s=1}^{m-1} T_{y_s^t,y_{s+1}^t}
\right) \\
&=
\grad_{\wvec_c}
\sum_{s=1}^m \inner{\wvec_{y_s^t}}{\xvec_s^t}
\end{align}
since the transition term does not depend on $\wvec_c$. For each position $s$,
\begin{align}
\grad_{\wvec_c}\inner{\wvec_{y_s^t}}{\xvec_s^t}
=
\llbracket y_s^t = c \rrbracket \xvec_s^t.
\end{align}
Hence,
\begin{align}
\grad_{\wvec_c} S(X^t,\yvec^t)
=
\sum_{s=1}^m \llbracket y_s^t = c \rrbracket \xvec_s^t.
\end{align}
...
\grad_{\wvec_c}\log Z_{X^t}
&=
\frac{1}{Z_{X^t}} \grad_{\wvec_c} Z_{X^t} \\
&=
\frac{1}{Z_{X^t}}
\grad_{\wvec_c}
\sum_{\hat{\yvec}\in\Ycal^m}
\exp\!\bigl(S(X^t,\hat{\yvec})\bigr) \\
&=
\frac{1}{Z_{X^t}}
\sum_{\hat{\yvec}\in\Ycal^m}
\exp\!\bigl(S(X^t,\hat{\yvec})\bigr)
\grad_{\wvec_c} S(X^t,\hat{\yvec}).
\end{align}
Also,
\begin{align}
\grad_{\wvec_c} S(X^t,\hat{\yvec})
=
\sum_{s=1}^m \llbracket \hat y_s = c \rrbracket \xvec_s^t.
\end{align}
Substituting,
\begin{align}
\grad_{\wvec_c}\log Z_{X^t}
...
\left(
\sum_{\hat{\yvec}\in\Ycal^m}
p(\hat{\yvec}\mid X^t)\llbracket \hat y_s = c \rrbracket
\right)\xvec_s^t \\
&=
\sum_{s=1}^m p(y_s = c \mid X^t)\xvec_s^t,
\end{align}
where the last equality uses the definition of the marginal probability.
...
\sum_{s=1}^m
\bigl(
\llbracket y_s^t = c \rrbracket - p(y_s = c \mid X^t)
\bigr)\xvec_s^t.
\end{align}

\paragraph{Derivative with respect to $T_{ij}$.}
Similarly,
\begin{align}
\grad_{T_{ij}}\log p(\yvec^t \mid X^t)
=
\grad_{T_{ij}} S(X^t,\yvec^t)
-
\grad_{T_{ij}}\log Z_{X^t}.
\end{align}
...
&=
\sum_{s=1}^{m-1}
\llbracket (y_s^t,y_{s+1}^t) = (i,j) \rrbracket.
\end{align}
...
\sum_{\hat{\yvec}\in\Ycal^m}
\exp\!\bigl(S(X^t,\hat{\yvec})\bigr)
\grad_{T_{ij}} S(X^t,\hat{\yvec}) \\
&=
\sum_{\hat{\yvec}\in\Ycal^m}
p(\hat{\yvec}\mid X^t)
\sum_{s=1}^{m-1}
\llbracket (\hat y_s,\hat y_{s+1}) = (i,j) \rrbracket \\
&=
\sum_{s=1}^{m-1}
\left(
\sum_{\hat{\yvec}\in\Ycal^m}
...
&=
\sum_{s=1}^{m-1}
p(y_s=i,y_{s+1}=j \mid X^t),
\end{align}
where the last equality is the pairwise marginal probability.
...
\paragraph{(1b) Gradient of $\log Z_X$ as an expectation of features.}
Recall
\begin{align}
Z_X
=
\sum_{\hat{\yvec}\in\Ycal^m}
\exp\!\left(
\sum_{s=1}^m \inner{\wvec_{\hat y_s}}{\xvec_s}
+
\sum_{s=1}^{m-1} T_{\hat y_s,\hat y_{s+1}}
\right).
\end{align}
We show that the gradient of $\log Z_X$ with respect to $\wvec_c$ and $T_{ij}$ is the expectation of suitable feature functions under $p(\yvec\mid X)$.

\paragraph{Gradient with respect to $\wvec_c$.}
Define the node feature
\begin{align}
f_c(X,\yvec)
:=
\sum_{s=1}^m \llbracket y_s = c \rrbracket \xvec_s
\in \RR^{128}.
\end{align}
Then
\begin{align}
\grad_{\wvec_c}\log Z_X
&=
\frac{1}{Z_X}\grad_{\wvec_c} Z_X \\
&=
\frac{1}{Z_X}
\sum_{\hat{\yvec}\in\Ycal^m}
\exp\!\left(
...
\sum_{s=1}^{m-1} T_{\hat y_s,\hat y_{s+1}}
\right)
\sum_{s=1}^m \llbracket \hat y_s = c \rrbracket \xvec_s \\
&=
\sum_{\hat{\yvec}\in\Ycal^m}
p(\hat{\yvec}\mid X)
\sum_{s=1}^m \llbracket \hat y_s = c \rrbracket \xvec_s \\
&=
\Exp_{p(\yvec\mid X)}\!\left[
\sum_{s=1}^m \llbracket y_s = c \rrbracket \xvec_s
\right] \\
&=
\Exp_{p(\yvec\mid X)}\!\left[f_c(X,\yvec)\right].
\end{align}
Thus, $\grad_{\wvec_c}\log Z_X$ is the expectation of the node feature $f_c(X,\yvec)$.

\paragraph{Gradient with respect to $T_{ij}$.}
Using the hint,
\begin{align}
T_{y_s,y_{s+1}}
=
\sum_{p\in\Ycal}\sum_{q\in\Ycal}
T_{pq}\,\llbracket (y_s,y_{s+1})=(p,q)\rrbracket.
\end{align}
Hence, for fixed $(i,j)$, define the edge feature
\begin{align}
g_{ij}(X,\yvec)
:=
\sum_{s=1}^{m-1}
\llbracket (y_s,y_{s+1})=(i,j)\rrbracket.
\end{align}
Then
\begin{align}
\grad_{T_{ij}}\log Z_X
&=
\frac{1}{Z_X}\grad_{T_{ij}} Z_X \\
&=
\frac{1}{Z_X}
\sum_{\hat{\yvec}\in\Ycal^m}
\exp\!\left(
...
\right] \\
&=
\Exp_{p(\yvec\mid X)}\!\left[g_{ij}(X,\yvec)\right].
\end{align}
Thus, $\grad_{T_{ij}}\log Z_X$ is the expectation of the edge feature $g_{ij}(X,\yvec)$.
...
\grad_{\wvec_c}\log Z_X
&=
\Exp_{p(\yvec\mid X)}[f_c(X,\yvec)],
\\
\grad_{T_{ij}}\log Z_X
&=
\Exp_{p(\yvec\mid X)}[g_{ij}(X,\yvec)].
\end{align}
Equivalently, the gradient of $\log Z_X$ is the expected feature count under the model distribution $p(\yvec\mid X)$.
...
\paragraph{(2a):} The value of $\frac{1}{n} \sum_{t=1}^n \log p(\yvec^t | X^t)$ we got is $-31.288437439649147$.
\paragraph{(2b):} The optimal objective value of \eqref{eq:obj_MLE} we got is $3701.1581395300996$.
...
\begin{itemize}
	...
	We evaluated all three models on $C \in \{1, 10, 100, 1000\}$. The resulting letter-wise accuracy plots are shown in Figures~\ref{fig:q3a_crf_letter}, \ref{fig:q3a_svmmc_letter}, and \ref{fig:q3a_svmstruct_letter}.
	\begin{figure}[H]
	    \centering
	    \begin{minipage}[b]{0.32\textwidth}
	        \centering
	        \includegraphics[width=\textwidth]{../result/other/benchmark/crf_letter_acc.png}
	        \caption{CRF letter-wise accuracy}
	        \label{fig:q3a_crf_letter}
	    \end{minipage}
	    \hfill
	    \begin{minipage}[b]{0.32\textwidth}
	        \centering
	        \includegraphics[width=\textwidth]{../result/other/benchmark/svm_mc_letter_acc.png}
	        \caption{SVM-MC letter-wise accuracy}
	        \label{fig:q3a_svmmc_letter}
	    \end{minipage}
	    \hfill
	    \begin{minipage}[b]{0.32\textwidth}
	        \centering
	        \includegraphics[width=\textwidth]{../result/other/benchmark/svm_struct_svmhmm_letter_acc.png}
	        \caption{SVM-Struct letter-wise accuracy}
	        \label{fig:q3a_svmstruct_letter}
	    \end{minipage}
	\end{figure}
	The corresponding letter-wise accuracies are:
	\[
	\begin{array}{c|cccc}
	C & 1 & 10 & 100 & 1000 \\
	\hline
	\text{CRF} & 0.4483 & 0.6915 & 0.8056 & 0.8375 \\
	\text{SVM-MC} & 0.6998 & 0.6996 & 0.6993 & 0.6992 \\
	\text{SVM-Struct} & 0.6757 & 0.7534 & 0.8224 & 0.8462
	\end{array}
	\]
	The main observation is that CRF and SVM-Struct both benefit strongly from increasing $C$, while SVM-MC is nearly flat and even slightly worse as $C$ increases. The best letter-wise results are achieved by SVM-Struct at $C=1000$ with accuracy $0.8462$, followed closely by CRF at $C=1000$ with accuracy $0.8375$. SVM-MC remains around $0.70$ regardless of $C$, showing that an independent-letter classifier cannot exploit sequence structure.
	...
	The word-wise accuracy plots are shown in Figures~\ref{fig:q3b_crf_word}, \ref{fig:q3b_svmmc_word}, and \ref{fig:q3b_svmstruct_word}.
	\begin{figure}[H]
	    \centering
	    \begin{minipage}[b]{0.32\textwidth}
	        \centering
	        \includegraphics[width=\textwidth]{../result/other/benchmark/crf_word_acc.png}
	        \caption{CRF word-wise accuracy}
	        \label{fig:q3b_crf_word}
	    \end{minipage}
	    \hfill
	    \begin{minipage}[b]{0.32\textwidth}
	        \centering
	        \includegraphics[width=\textwidth]{../result/other/benchmark/svm_mc_word_acc.png}
	        \caption{SVM-MC word-wise accuracy}
	        \label{fig:q3b_svmmc_word}
	    \end{minipage}
	    \hfill
	    \begin{minipage}[b]{0.32\textwidth}
	        \centering
	        \includegraphics[width=\textwidth]{../result/other/benchmark/svm_struct_svmhmm_word_acc.png}
	        \caption{SVM-Struct word-wise accuracy}
	        \label{fig:q3b_svmstruct_word}
	    \end{minipage}
	\end{figure}
	The corresponding word-wise accuracies are:
	\[
	\begin{array}{c|cccc}
	C & 1 & 10 & 100 & 1000 \\
	\hline
	\text{CRF} & 0.0119 & 0.1620 & 0.3812 & 0.4725 \\
	\text{SVM-MC} & 0.1736 & 0.1713 & 0.1721 & 0.1701 \\
	\text{SVM-Struct} & 0.1663 & 0.2623 & 0.4135 & 0.4827
	\end{array}
	\]
	At the word level, the advantage of structured models is even clearer. SVM-MC stays around $0.17$, which is much lower than its letter-wise accuracy because one wrong letter makes the whole word incorrect. In contrast, CRF and SVM-Struct improve substantially as $C$ increases, reaching $0.4725$ and $0.4827$ respectively at $C=1000$. This shows that modeling transitions between neighboring letters provides a large benefit, especially for full-word prediction.
...
For the following results in this problem, we select $C = 1000$ based on the previous problem.

\paragraph{(4a):}
\noindent \textbf{Note:} In figure~\ref{fig:4alr1} to figure~\ref{fig:4am3}, all the graphs contain accuracy instead of error. For analysis and reasoning on the hyperparameters, this is fine. In our final graph (figure~\ref{fig:final4a}), we will use error as the problem requires.\\
From our experiment, the learning rate is the most important factor for the stochastic gradient descent approaches. We noticed that for the learning rate of $0.1$ and $0.01$ the SGD's objective is very unstable as it tends to diverge away from the initial objective value. The value of $0.001$ is better, but the objective values is still noisy and does not converge near optimal values found by LBFGS. The learning rate between $0.0005$ and $0.0001$ is ideal as the objective values converge quickly to almost optimal.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B10_lr0_001_mom0_2.png}
    \caption{$B=10$, learning rate $=0.001$, momentum $=0.2$}
    \label{fig:4alr1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B10_lr0_0003_mom0_2.png}
    \caption{$B=10$, learning rate $=0.0003$, momentum $=0.2$}
    \label{fig:4alr2}
\end{figure}
\noindent Notice in the above graphs, the learning rate of $0.001$ still produce a lot of noises to the objective function, while the curve for learning rate of $0.0003$ is smoother.\\
\noindent Another trend we find is that values of $B$ between $10$ and $20$ tend to return objective value to be close to that of LBFGS. Lower value of $B$ such that $B=5$ makes the SGD's objective to oscillate as demonstrated in the following graphs:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B5_lr0_001_mom0_2.png}
    \caption{$B=5$, learning rate $=0.001$, momentum $=0.2$}
    \label{fig:4aB1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B20_lr0_001_mom0_2.png}
    \caption{$B=20$, learning rate $=0.001$, momentum $=0.2$}
    \label{fig:4aB2}
\end{figure}
\noindent Regarding momentum, large value such as 0.9 to sometimes even 0.5 cause noises and inability to converge close to optimal. Details can be found below for momentum value of 0.9, 0.5, and 0.2:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B2_lr0_0003_mom0_9.png}
    \caption{$B=2$, learning rate $=0.0003$, momentum $=0.9$}
    \label{fig:4am1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B2_lr0_0003_mom0_5.png}
    \caption{$B=2$, learning rate $=0.0003$, momentum $=0.5$}
    \label{fig:4am2}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/mcmc_performance_B2_lr0_0003_mom0_2.png}
    \caption{$B=2$, learning rate $=0.0003$, momentum $=0.2$}
    \label{fig:4am3}
\end{figure}
\noindent Thus, for our experiment, we decide to choose $B=10$, learning rate to be $0.0003$, and momentum to be $0.2$ to obtain the following graph:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{../code/other/khoi/generated_graphs/4a_mcmc_performance_B10_lr0_0003_mom0_2.png}
    \caption{Final selection: $B=10$, learning rate $=0.0003$, momentum $=0.2$}
    \label{fig:final4a}
\end{figure}
\noindent We can see that the SGD and SGD with momentum learning objective and word-wise error drops within the first few passes. However, both methods struggle to break the barrier to obtain lower objective and error than the LBFGS method that steadily converges. Another note is that there is a sudden drop at the beginning for both SGD methods. This is because a pass of SGD contains multiple rounds of sampling, leading do the sudden drop.

\paragraph{(4b):} 
\subsubsection*{(i):} Notice that the $y_s$ only depends on its neighbor, namely $y_{s-1}$ and $y_{s+1}$. Thus, the probability we are looking for only depend on the node potential and its two edge potentials. In other words,
\begin{align*}
    p(y_s|\ldots, y_{s-3}, y_{s-1}, y_{s+1}, y_{s+3}, \ldots, X) &= p(y_s|y_{s-1}, y_{s+1}, X) \\
    &= \frac{e^{(\langle w_{y_s}, x_s \rangle + T_{y_{s-1}, y_s} + T_{y_s, y_{s+1}})}}{\sum_{c \in \mathcal{Y}} e^{(\langle w_c, x_s \rangle + T_{y_{s-1}, c} + T_{c, y_{s+1}})}}
\end{align*}
To calculate such distribution, notice that the numerator is part of the denominator. The denominator can then be evaluate by brute force by iterating over every letter of a word and every letter in the alphabet. Thus, the runtime to evaluate this distribution is $O(m|\mathcal{Y}|)$.
\subsubsection*{(ii):}
The first thing to mention is that LBFGS of scipy works well only with non-stochastic evaluation. Multi-batches LBFGS is possible as of research in 2016, but we will not use it here. Therefore, it is expected that sampling will not work well here. Indeed, for low value of $S$ such as $S = 5$ or $S = 10$, the objective function either fails linear search or converge at a very bad solution:
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/LBFGS_S_5.png}
        % \caption{Caption for Image 1}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/LBFGS_S_10.png}
        % \caption{Caption for Image 2}
    \end{minipage}
\end{figure}
\noindent For the LBFGS with larger value of $S$, in our case $50$, the solution converge slowly, but takes a very long time. In the following screenshot:
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/LBFGS_S_50.png}
        % \caption{Caption for Image 1}
    \end{minipage}
\end{figure}
\noindent We observe that there is a steady decrease of both the objective function and word-wise error. However, it takes roughly an hour to get to iteration 10 and 90 evaluations. In addition, this is only achieved when we tweak the value of eta to be $0.1$ and stepmx to be $1$ of function fmin\_tnc. Thus, we conclude the execution.\\
Regarding the performance for the MCMC and RB MCMC, we produced the following graph for $S=2$, $S=5$, $S=10$, and $S=50$ using the same parameter from part 4a. The result are shown as follows:
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_2_Objective.png}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_2_Error.png}
    \end{minipage}
    \caption{$S=2$}
\end{figure}
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_5_Objective.png}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_5_Error.png}
    \end{minipage}
    \caption{$S=5$}
\end{figure}
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_10_Objective.png}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_10_Error.png}
    \end{minipage}
    \caption{$S=10$}
\end{figure}
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_50_Objective.png}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_50_Error.png}
    \end{minipage}
    \caption{$S=50$}
\end{figure}
\noindent We notice that compared to Figure~\ref{fig:4alr2}, adding stochastic selection for the gradient introduces some noises to the objective and especially the error. In addition, we can see that as $S$ increases, the noise reduces, leading to a smoother curve of the objective, though the error is still somewhat noisy. In addition, larger values of $S$ also lower the overall objective and error rate. This makes sense because as the number of sample increases, the gradient obtained by sampling gets closer to the true gradient obtained from the dynamic programming method. Thus, it will guide the SGD methods to better solutions.\\
We can see the reduction in the noise and value of the objective by taking the log of the objective:
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_2_Objective_Log.png}
        \caption{log objective for $S=2$}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_5_Objective_Log.png}
        \caption{log objective for $S=5$}
    \end{minipage}
\end{figure}
\begin{figure}[H]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_10_Objective_Log.png}
        \caption{log objective for $S=10$}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../code/other/khoi/generated_graphs/S_50_Objective_Log.png}
        \caption{log objective for $S=50$}
    \end{minipage}
\end{figure}

\paragraph{(4c):}
\subsubsection*{(i):} The implementation of RB sampling can be expressed as follows:
	
	1. Sample $y_s$ for all $s$ based on the independent distribution given that $T = 0$.
	
	2. Set the count of $y_s = c$ for all $s, c$ to 0 and $y_s, y_{s+1} = p, q$ to 0 for all $s, p, q$.
	
	3. For $k = 1, 2, \ldots, S$
	
	4. $\qquad$ Even indexed sampling: For all even index $y_s$, compute the conditional distribution $y_s = c$ for all $c = \{1, \ldots, 26\}$. Then, increase the count for $y_s = c$ by $p_c$ and count of $T_{p_{s-1}, p_s=c}$ and $T_{p_s=c, p_{s+1}}$ by $p_c$.. Finally, draw sample on $y_s$ based on the distribution.
	
	5. $\qquad$ Odd indexed sampling: Do the same thing as even index.
	
	7. Normalize by dividing the node count by $|S|$ and the edge count by $2|S|$
	
\noindent Note that the reason we divide the node count by $|S|$ is that each node is increases by some $p_c$ once, while for edge count it is $2|S|$ since it is double counted. In addition, we can draw sample at the end of each loop, but we can also do it separately in line 4 and 5 because the distribution are independent. We think that the count should be update right before the sampling because it will capture the relationship of the current node and the state it is in, while updating the count outside the loop is basically taking the average from the sample, which still allows room for high variance.
\subsubsection*{(ii):}
Using $S$ to be $\{5, 10, 50, 100, 500, 1000\}$, we obtained the following graph:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{../code/other/khoi/generated_graphs/kl_divergence_comparison.png}
    \caption{KL divergence comparison of MCMC method and RB MCMC method for nodes and edges.}
    \label{fig:4cii}
\end{figure}
\noindent All four lines show that our sampling method work because the KL divergence against the DP result decreases towards 0 as $S$ increases. In addition, notice that with RB, the KL divergence is always smaller than the MCMC method in part 4b for all value of $S$ we used, indicating that RB sampling is more effective.
...
\begin{itemize}
	...
	We used the best values found in Question 3(a), namely $C=1000$ for CRF and $C=1$ for SVM-MC. The resulting letter-wise accuracy plot is shown in Figure~\ref{fig:q5_letter_acc}. The corresponding values are:
	\[
	\begin{array}{c|ccccc}
	x & 0 & 500 & 1000 & 1500 & 2000 \\
	\hline
	\text{CRF} & 0.8375 & 0.8091 & 0.7843 & 0.7612 & 0.7318 \\
	\text{SVM-MC} & 0.6996 & 0.6683 & 0.6443 & 0.6213 & 0.5956
	\end{array}
	\]
	\begin{figure}[H]
	    \centering
	    \includegraphics[width=0.68\linewidth]{../code/results/q5_letter_accuracy.png}
	    \caption{Question 5(a): letter-wise test accuracy versus number of applied training distortions.}
	    \label{fig:q5_letter_acc}
	\end{figure}
	The main observation is that both models degrade as more distorted training words are introduced, but the CRF is consistently more robust than SVM-MC. Even at $x=2000$, the CRF still achieves $0.7318$ letter-wise accuracy, whereas SVM-MC drops to $0.5956$. This suggests that the structured CRF model benefits from sequence context and is less sensitive to distortion in the letter images than the independent-letter SVM-MC baseline.
	...
	For Question 5(b), we again used $C=1000$ for CRF and $C=1$ for SVM-MC. The resulting word-wise accuracy plot is shown in Figure~\ref{fig:q5_word_acc}. The measured values are:
	\[
	\begin{array}{c|ccccc}
	x & 0 & 500 & 1000 & 1500 & 2000 \\
	\hline
	\text{CRF} & 0.4722 & 0.4085 & 0.3597 & 0.3260 & 0.2876 \\
	\text{SVM-MC} & 0.1727 & 0.1378 & 0.1181 & 0.0980 & 0.0814
	\end{array}
	\]
	\begin{figure}[H]
	    \centering
	    \includegraphics[width=0.68\linewidth]{../code/results/q5_word_accuracy.png}
	    \caption{Question 5(b): word-wise test accuracy versus number of applied training distortions.}
	    \label{fig:q5_word_acc}
	\end{figure}
	The same trend is even more pronounced for word-wise accuracy. Since one incorrect letter makes the whole word incorrect, SVM-MC suffers much more severely, falling from $0.1727$ to $0.0814$. In contrast, the CRF drops from $0.4722$ to $0.2876$ but remains substantially stronger at every distortion level. Therefore, the structured model is clearly more robust to training-image distortions, especially when evaluated at the word level.
